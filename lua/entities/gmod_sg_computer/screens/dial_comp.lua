...
function ENT:InitializeDialScreen()
	--Dial screen
	-- Blinking boxes
	self.Boxes1 = {}
	self.Boxes2 = {}
...
function ENT:DrawDialScreen()
	local Alpha = math.abs(math.sin(CurTime()*math.pi/2))
	local NLocal = not self:GetNW2Bool("Local",false)
	...
	for i=1,9	do
			...
			local active = self:GetNW2String("Chevrons")[i == 9 and 7 or i>5 and i-2 or i > 3 and i+4 or i] == "1" or self.Error
	...
	if not self:GetNW2Bool("Inbound",false) then
	...
	for i=1,#self.DialingAddress + (self.Timer8 and #self.DialingAddress < 8 and 1 or 0) do
		if NLocal then
				local tm8 = self.Timer8 and math.min(1,CurTime()-self.Timer8)*3
				draw.SimpleText(i, "Marlett_29", 436,15+i*39, i == 8 and tm8 and Red or self.SecondColor, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	...
	elseif self:GetNW2Bool("Inbound",false) then
		surface.SetDrawColor(Red)
...
function ENT:ThinkDialScreen()
	...
	local chevron = self:GetNW2Int("Chevron",0)
	local locked = self:GetNW2Bool("ChevronLocked",false)
	...
	for i=1,9 do
			if CurTime() - self.GradientsTimers[i] > self.GradientSpeeds[i] and active and (inbound or open or self:GetNW2Bool("RingRotation",false)) then
					self.GradientSpeeds[i] = math.Rand(0.4,0.8)
	...
	elseif (self.Dialed or self.Locked) then
		if not open and not inbound and chevron <= 0 then -- we fail dial
			print(chevron)
	...
	if (not self.SymbolAnim and not self.SymbolAnim2 or self.SymbolAnim2 and (CurTime()-self.SymbolAnim2) > 0.6) and not self.Error then
			...
			if LastSecond and dialadd[#dialadd] ~= "#" and chevron < 9 then
					...
					if active and not open and locked then
						...
						self.DialingAddress = dialadd..smadd
					elseif dialadd ~= "" or not self.Error then
	...
	if not self:GetNW2Bool("Local",false) and not inbound and (#dialadd == 7 and not LastChev or #dialadd > 7) and not self.Timer8 then
		self.Timer8 = CurTime()
	end
	if (self:GetNW2Bool("Local",false) or #self.DialingAddress < 7 or inbound) and self.Timer8 then
		self.Timer8 = false
	end
end
ENT.ScreenFunctions[1] = {ENT.InitializeDialScreen,ENT.DrawDialScreen,ENT.ThinkDialScreen}
