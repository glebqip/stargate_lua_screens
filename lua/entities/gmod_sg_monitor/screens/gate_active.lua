...
if SERVER then
      ...
      if self.SelfDestructState == 2 then
        ...
        if not server.SelfDestruct then
          ...
          for k,v in pairs(server.SelfDestructCodes) do
            if v[1]==self.SelfDestructCode then
              self.SelfDestructName = v[2]
              good = true
              break
            end
          end
  ...
  function SCR:Trigger(curr,key,value)
    ...
    if key == 33 and self.Entity.Keys[154] and value and not self.SDOffTimer then
    ...
    if self.SelfDestructState == 1 then
      ...
      if key == 13 and value and #self.SelfDestructCode >= 8 then
        self.SelfDestructState = 2
    ...
    if self.Entity.Server.SelfDestruct then
      ...
      if key == 13 and value and #self.SelfDestructResetCode >= 8 then
        local server = self.Entity.Server
        local good = false
        --Check for good code
        for k,v in pairs(server.SelfDestructResetCodes) do
          if v[1]==self.SelfDestructResetCode then
            self.SelfDestructResetName = v[2]
            good = true
            break
          end
        end
